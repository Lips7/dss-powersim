...
def sim_data(n_subj=25, n_pop=15, n_rock=15, beta_0=60, beta_1=5, omega_0=3, tau_0=7, tau_1=4, rho=0.2, sigma=8):
    random_effects = np.repeat(np.random.multivariate_normal([0, 0], [[tau_0**2, rho*tau_0*tau_1], [rho*tau_0*tau_1, tau_1**2]], size=n_subj, tol=1e-6), n_pop + n_rock, axis=0)

    subj_id = np.repeat(np.arange(1, n_subj + 1), n_pop + n_rock)
    song_id = np.tile(np.arange(n_pop + n_rock), n_subj)
    category = np.tile(np.repeat(["pop", "rock"], [n_pop, n_rock]), n_subj)
    genre_i = np.tile(np.repeat([0, 1], [n_pop, n_rock]), n_subj)
    O_0i = np.tile(np.random.normal(0, omega_0, n_pop + n_rock), n_subj)
    T_0j = random_effects[:, 0]
    T_1j = random_effects[:, 1]
    e_ij = np.random.normal(0, sigma, (n_pop + n_rock) * n_subj)
